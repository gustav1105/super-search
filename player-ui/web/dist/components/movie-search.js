...
export function submitSearchForm(gridContainer) {
  ...
  searchForm.addEventListener("submit", /*#__PURE__*/function () {
    var _ref = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(event) {
      var errorContainer, queryInput, propertySelect, query, property, response, data, firstCard;
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          case 0:
            ...
            propertySelect = document.getElementById("property");
            query = queryInput.value;
            property = propertySelect.value; // Clear gridContainer but keep searchForm intact
            gridContainer.replaceChildren(searchForm);

            // Clear the grid container before fetching new results
            _context.prev = 7;
            _context.next = 10;
            return fetch('http://192.168.10.94:8000/query', {
              method: 'POST',
              headers: {
          ...
          case 15:
            ...
            if (data.results && data.results.length > 0) {
              data.results.forEach(function (item, i) {
                var movie = item.metadata;
                var card = createCard(movie);
                card.setAttribute('data-index', i);
                card.setAttribute('tabindex', 0);
                gridContainer.appendChild(card);
              });
              firstCard = gridContainer.querySelector('[data-index="0"]');
              if (firstCard) {
                firstCard.focus();
              }
            } else {
          ...
          case 19:
            ...
            errorContainer.textContent = "Error: ".concat(_context.t0.message);
          case 22:
          case "end":
...
function getValueByOption(option) {
  switch (option) {
    ...
    default:
      return 500;
    // Default value if the option doesn't match
  }
}